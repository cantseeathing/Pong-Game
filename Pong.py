...
class Pong:
    ...
    def ball_movement(self):
        ...
        ball_angle_threshold = 10
        ball_angle_variation = 30
        if self.first_ball_kick:
            self.ball_object.goto(x=0, y=0)
            if random.randint(0, 1):
                random_dir = random.randint(-50, 50)
            else:
                random_dir = random.randint(130, 230)
            self.ball_object.setheading(random_dir)
        ...
        if self.ball_object.ycor() >= self.game_limit_2.ycor() * 0.95 or \
            ...
            self.ball_object.setheading(-1 * self.ball_object.heading())
        if (self.ball_object.distance(self.player_paddle) < ball_distance and self.ball_object.xcor() >=
            ...
            print(f"collision with paddle at angle {self.ball_object.heading()}")
            # if ball_angle_threshold + 90 >= self.ball_object.heading() >= \
            #         ball_angle_threshold - 90:
            #     print(self.ball_object.heading())
            #     if self.ball_object.heading() > 0:
            #         self.ball_object.setheading(self.ball_object.heading()
            #                                     + ball_angle_threshold * random.randint(0, ball_angle_variation))
            #     else:
            #         self.ball_object.setheading(self.ball_object.heading()
            #                                     + ball_angle_threshold * random.randint(-ball_angle_variation, 0))
            #     print("90 degree collision")
            #     print(self.ball_object.heading())
            # else:
            #     self.ball_object.setheading(180 + self.ball_object.heading())
            self.ball_object.setheading(180 + self.ball_object.heading())
            if 0 + ball_angle_threshold > self.ball_object.heading() > 0 - ball_angle_threshold:
                print("bad angle detected")
                self.ball_object.setheading(random.randint(0, ball_angle_variation) + self.ball_object.heading())
            elif 180 - ball_angle_threshold < self.ball_object.heading() < 180 + ball_angle_threshold:
                print("bad angle detected")
                self.ball_object.setheading(random.randint(-ball_angle_variation, 0) + self.ball_object.heading())
        self.ball_object.forward(self.ball_speed)
